...
function onBodyReady(){
				...
				for(idx in data){
					...
					if(data[idx][0] == "No Projects Available. Try make one!"){
						...
						for(i in data[idx]){
							...
							html +='<li><a href="#" id="'+pid+'"><h2>';
							html += pname;
							html += '</h2><small> created by: '+name+'</small>';
							html +='</li>';
						}
					}
				}
	...
	$("#create").live("click", function(){
		...
		if(project_name.length > 0){
			$.ajax({
				url: 'http://www.nciairsoft.com/aria/create_project.php',
	...
	$("#invite").live("click", function(){
		...
		if(valid_creds){
			$.ajax({
				url: 'http://www.nciairsoft.com/aria/invite.php',
	...
	$("#update_details").live("click", function(){
		...
		if(valid_creds){
			$.ajax({
				url: 'http://www.nciairsoft.com/aria/update_details.php',
				data: {email:email,fname:fname,sname:sname,pwd:pass},
				type: 'POST',
				success: function(data){
				   if(data == 1){
					alert("Thanks for updating your details! Go have fun!");
					window.location.href="project_splash.html";
				   }else{
					alert("Sorry something went wrong. Try enter your details again.");
				   }
				}
			});
		}else{
